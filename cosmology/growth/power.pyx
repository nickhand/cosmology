...
class Power(object):
    ...
    def _CAMB_transfer(self):
        try:
            ...
            if np.amax(self.k) > np.amax(k):
                ...
                hik_extrap = functionator.powerLawExtrapolator(gamma=p_gamma, A=p_amp)
            ...
            if np.amin(self.k) < np.amin(k):
                ...
                lok_extrap = functionator.powerLawExtrapolator(gamma=p_gamma, A=p_amp)
    ...
    def nonlinear_power(self):
        """
        Non-linear power [units :math:`Mpc^3/h^3`]
